...
void AssertWithSideEffects::VisitStmt(Stmt *stm)
{
    ...
    if (Utils::childsHaveSideEffects(co->getCond())) {
        Utils::emitWarning(m_ci, co->getLocStart(), "Code inside Q_ASSERT has side-effects but won't be built in release mode [-Wmore-warnings-assert-with-side-effects]");
    }
}
