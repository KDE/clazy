...
void ModernizeListInitialization::VisitStmt(clang::Stmt *stmt)
{
    ...
    if (auto *topLevelOperatorCall = dyn_cast<CXXOperatorCallExpr>(stmt);
        topLevelOperatorCall && topLevelOperatorCall->getOperator() == OverloadedOperatorKind::OO_LessLess) {
        if (std::ranges::find(m_alreadyCheckedOperatorCalls, topLevelOperatorCall) != m_alreadyCheckedOperatorCalls.end()) {
            return;
        }
        std::string name = topLevelOperatorCall->getType().getAsString();
        if (name != "QStringList" && !name.starts_with("QList")) {
            return;
        }
        auto parent = m_context->parentMap->getParent(topLevelOperatorCall);
        if (!isa<ImplicitCastExpr>(parent)) {
            return;
        }
        if (auto bla = dyn_cast<MaterializeTemporaryExpr>(topLevelOperatorCall->getArg(0))) {
        ...
        checkOperatorCallListInitialization({parent->getBeginLoc(), topLevelOperatorCall->getEndLoc()}, topLevelOperatorCall);
    }
}
