...
#include "checks/level1/modernize-overloaded-connects.h"
...
#include "checks/manuallevel/cache-model-rolenames.h"
...
void CheckManager::registerChecks()
{
    ...
    registerCheck(check<CacheModelRolenames>("cache-model-rolenames", ManualCheckLevel, RegisteredCheck::Option_VisitsDecls, false));
    registerCheck(check<CompareMemberCheck>("compare-member-check", ManualCheckLevel, RegisteredCheck::Option_None, true));
    ...
    registerCheck(check<ModernizeOverloadedConnects>("modernize-overloaded-connects", CheckLevel1, RegisteredCheck::Option_VisitsStmts, false));
